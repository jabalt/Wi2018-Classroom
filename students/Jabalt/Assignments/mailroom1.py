...
donors = ["Bill Gates", "Paul Allen", "Jeff Bezos", "Mark Zuckerberg", "Warren Buffet"]
amounts = [3.237,460.45,6.42,506.191,8.330]
num_gifts = [1,2,1,1,1]
choice =" "
def mailroom():
    """This part automates mail sending and report generation """
    choice =" "
    while choice != "3":
        ...
        if choice == "1":
            ...
            print("Dear {0},\n \tThank you for your generous donation of ${1:.2f}. We are very grateful for you contribution! \n-Team".format(name, amounts[donors.index(name)]))

        if choice == "2":
            print("{:20}|{:^18}|{:^13}|{:>18}".format("Donor Name", "Total Given", "Num Gifts", "Average Gift"))
            for donor in donors:
                print("{:20}${:>18} {:>12} ${:>16}".format(donor, amounts[donors.index(donor)], num_gifts[donors.index(donor)],(amounts[donors.index(donor)]/num_gifts[donors.index(donor)])))
